...
class GlobalFunctions:
    def createFile(file_name):
        with open(GlobalVariable.TsvFilePath + file_name + '.tsv', 'wt') as csvfile:
            # print(GlobalVariable.TsvFilePath)
            tsv_writer = csv.writer(csvfile, delimiter='\t')
            tsv_writer.writerow(
                ['scrappedUrl', 'eventname', 'startdate', 'enddate', 'timing', 'eventinfo', 'ticketlist', 'orgProfile',
                 'orgname', 'orgweb', 'logo', 'sposor', 'agendalist', 'type', 'category', 'city', 'country', 'venue',
                 'event_website', 'googlePlaceUrl', 'contact', 'speaker', 'emode'])
    ...
    def date_converter(dat):
        if dat.startswith('-'):
            dat = dat[1:]
        dat = dat.replace('&', '-').replace(' and ', ' - ').replace('ugust', 'u-g-u-s-t').replace('st', '').replace(
            'u-g-u-s-t', 'ugust').replace('turday', 't-u-r-d-a-y').replace('rd', '').replace('t-u-r-d-a-y',
                                                                                             'turday').replace('th',
                                                                                                               '').replace(
            'nday', 'n-d-a-y').replace('nd', '').replace('n-d-a-y', 'nday').title()
        ...
        if any(ele in dat for ele in [date(2000, 5, d).strftime('%a') for d in range(1, 8)]):
            ...
            pass
        dat = dat.replace('–', '-').replace('|', '').replace('to', '-').replace('TO', '-').strip().replace('c-ber',
                                                                                                           'ctober').replace(
            'C-BER', 'CTOBER')
        if '(' in dat:
        ...
        if dat == '':
            ...
            if '-' in mad:
                st = mad.split('-')[0].strip().replace(',', '').replace('nd', '').replace('st', '').replace('th',
                                                                                                            '').replace(
                    'rd', '').replace('ugu', 'ugust').strip()
                en = mad.split('-')[1].strip().replace(',', '').replace('nd', '').replace('st', '').replace('th',
                                                                                                            '').replace(
                    'rd', '').replace('ugu', 'ugust').strip()

            elif 'and' in mad:
                st = mad.split('and')[0].strip().replace(',', '').replace('nd', '').replace('st', '').replace('th',
                                                                                                              '').replace(
                    'rd', '').replace('ugu', 'ugust').strip()
                en = mad.split('and')[1].strip().replace(',', '').replace('nd', '').replace('st', '').replace('th',
                                                                                                              '').replace(
                    'rd', '').replace('ugu', 'ugust').strip()
                ...
                st = mad.strip().replace(',', '').replace('nd', '').replace('st', '').replace('th', '').replace('rd',
                                                                                                                '').replace(
                    'ugu', 'ugust').strip()
                en = mad.strip().replace(',', '').replace('nd', '').replace('st', '').replace('th', '').replace('rd',
                                                                                                                '').replace(
                    'ugu', 'ugust').strip()
            ...
            if re.search('\d{1,2}\.+\s?\d{1,2}\.?\s?\-\s?\d{1,2}\.?\d{1,2}\.?\d{4}', mad.strip()):
                #                 print(mad,'kiy')
                start = mad.split('-')[0].split('.')[0].strip() + ' ' + datetime.strptime(
                    mad.split('-')[0].split('.')[1].strip(), '%m').strftime('%B') + ' ' + mad.split('-')[1].split('.')[
                            2].strip()
                end = mad.split('-')[1].split('.')[0].strip() + ' ' + datetime.strptime(
                    mad.split('-')[1].split('.')[1].strip(), '%m').strftime('%B') + ' ' + mad.split('-')[1].split('.')[
                          2].strip()
            ...
            #                 09.09.2022(single)dat
                ...
                start = end = mad.split('.')[0].strip() + ' ' + datetime.strptime(mad.split('.')[1].strip(),
                                                                                  '%m').strftime('%B') + ' ' + \
                              mad.split('.')[2].strip()
